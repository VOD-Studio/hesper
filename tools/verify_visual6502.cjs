...
function reference(spec) {
  ...
  for (const name of [
    ...
    vm.runInContext(sources.find(([file]) => file === name)[1], c, {
      filename: name,
    });
  }
  ...
  const initial = {
    ...
    ram: c.memory.flatMap((value, address) =>
...
for (const [family, program, cycles, ram] of resetFamilies.filter(([family]) =>
  ["nop", "store", "rmw", "brk"].includes(family),
)) {
...
const usage =
  "Usage: node tools/verify_visual6502.cjs [--suite pins|reset|all] [--case NAME] [--record OUTPUT (requires --suite pins|reset, no --case)]";
const options = {};
const args = process.argv.slice(2);
for (let i = 0; i < args.length; i += 2) {
  if (
    !["--suite", "--case", "--record"].includes(args[i]) ||
    !args[i + 1] ||
    args[i + 1].startsWith("--") ||
    options[args[i]]
  ) {
...
for (const selected of suites) {
  ...
  if (!options["--record"]) {
    ...
    if (
      ...
      throw Error(`${selected.file}: fixture hash mismatch`);
  }
  ...
  if (options["--record"]) {
    ...
    const pinned = caseName
      ? JSON.parse(expected).filter((c) => c.name === caseName)
      : JSON.parse(expected);
