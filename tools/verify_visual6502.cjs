...
function reference(spec) {
    ...
    const p = spec.p ?? 0x20;
    const a = spec.a ?? 0, x = spec.x ?? 0, y = spec.y ?? 0, s = spec.s ?? 0xfd;
    const boot = [0xa2,s,0x9a,0xa2,x,0xa0,y,0xa9,p,0x48,0xa9,a,0x28,0x4c,start&255,start>>8];
    ...
    if (initial.pc !== c.readPC() || initial.a !== a || initial.x !== x || initial.y !== y || initial.s !== s || initial.p !== p) {
...
const resetHandler = [0x8d,0x10,0x20,0x8e,0x13,0x20,0x8c,0x14,0x20,0x08,0xba,0xe8,0x8e,0x11,0x20,0x68,0x8d,0x12,0x20,0x4c,0x47,0xb1];
const resetRam = [[0xfffc,0x34],[0xfffd,0xb1],[0x2000,0x63],[0x2010,0xcc],[0x2011,0xcc],[0x2012,0xcc],[0x2013,0xcc],[0x2014,0xcc],
    ...resetHandler.map((value,i) => [0xb134+i,value])];
const resetFamilies = [
    ['nop',[0xea,0xea],2,[],[]],
    ...
    ['rts',[0x60,0xea],6,[[0x1fe,0xff],[0x1ff,0x80]],[]],
    ['rti',[0x40,0xea],6,[[0x1fe,0xc9],[0x1ff,0],[0x100,0x81]],[]],
    ['brk',[0,0xea],7,[],[]],
    ['irq',[0xea,0xea],9,[],[[0,'irq',true]]],
    ['nmi',[0xea,0xea],9,[],[[0,'nmi',true]]],
];
function resetScenario(name, program, events, ram = [], p = 0x29) {
    resetScenarios.push({name:`reset-${name}`, program, ram:[...resetRam,...ram], p, events, cycles:64});
}
resetScenarios.push({name:'reset-registers-stack-wrap', program:[0xea,0xea], ram:resetRam,
    a:0x12, x:0x34, y:0x56, s:0x01, p:0x29, events:[[0,'res',true],[8,'res',false]], cycles:64});
for (const [family,program,cycles,ram,interrupt] of resetFamilies) {
    for (let half = 0; half < cycles*2; half++) for (const width of [8,9]) {
        resetScenario(`${family}-${half}-${width}`, program,
            [...interrupt,[half,'res',true],[half+width,'res',false]], ram);
    }
}
for (const half of [0,1]) {
    resetScenario(`held-${half}`, [0xea,0xea], [[half,'res',true]]);
    for (const release of [30,31]) {
        resetScenario(`long-${half}-${release}`, [0xea,0xea], [[half,'res',true],[release,'res',false]]);
    }
}
for (const [family,program,cycles,ram] of resetFamilies.filter(([family]) => ['nop','store','rmw','brk'].includes(family))) {
    for (const half of [0,1,cycles*2-2,cycles*2-1]) for (const width of [1,2,3]) {
        resetScenario(`pulse-${family}-${half}-${width}`, program, [[half,'res',true],[half+width,'res',false]], ram);
    }
}
// Reassert from release synchronization through stack reads, vector reads and fetch.
for (let half = 6; half < 26; half++) {
    resetScenario(`retrigger-${half}`, [0xea,0xea], [[0,'res',true],[4,'res',false],[half,'res',true],[half+8,'res',false]]);
}
for (const [family,program] of resetFamilies.filter(([family]) => ['nop','store','rmw'].includes(family))) {
    for (const half of [0,1,4,5,12,13,20,21,22,23,24,25]) {
        resetScenario(`rdy-${family}-${half}`, program,
            [[0,'res',true],[8,'res',false],[half,'rdy',true],[half+8,'rdy',false]]);
    }
}
for (const pin of ['irq','nmi']) for (const half of [0,1,6,7,8,9,14,15,20,21,22,23,24,25]) {
    resetScenario(`${pin}-overlap-${half}`, [0xea,0xea],
        [[0,'res',true],[8,'res',false],[half,pin,true],[half+4,pin,false]]);
}

const usage = 'Usage: node tools/verify_visual6502.cjs [--suite pins|reset|all] [--case NAME] [--record OUTPUT (requires --suite pins|reset, no --case)]';
const options = {};
const args = process.argv.slice(2);
for (let i = 0; i < args.length; i += 2) {
    if (!['--suite','--case','--record'].includes(args[i]) || !args[i+1] || args[i+1].startsWith('--') || options[args[i]]) {
        throw Error(usage);
    }
    options[args[i]] = args[i+1];
}
const suite = options['--suite'] ?? 'all';
if (!['pins','reset','all'].includes(suite) || (options['--record'] && (suite === 'all' || options['--case']))) throw Error(usage);
const suites = [
    {name:'pins', specs:scenarios, file:'pins.json', hash:manifest.fixture_sha256},
    {name:'reset', specs:resetScenarios, file:'reset.json', hash:manifest.reset_fixture_sha256},
].filter(s => suite === 'all' || suite === s.name);
const caseName = options['--case'];
if (caseName && !suites.some(s => s.specs.some(spec => spec.name === caseName))) throw Error(`No reference case matches ${JSON.stringify(caseName)} in suite ${suite}`);
for (const selected of suites) {
    const specs = selected.specs.filter(spec => !caseName || spec.name === caseName);
    if (!specs.length) continue;
    let expected;
    if (!options['--record']) {
        expected = fs.readFileSync(path.join(fixtures, selected.file), 'utf8');
        if (crypto.createHash('sha256').update(expected).digest('hex') !== selected.hash) throw Error(`${selected.file}: fixture hash mismatch`);
    }
    const actual = specs.map(reference);
    const serialized = '[\n' + actual.map(c => JSON.stringify(c)).join(',\n') + '\n]\n';
    if (options['--record']) {
        fs.writeFileSync(options['--record'], serialized);
        console.log(`Recorded ${actual.length} independent revD ${selected.name} traces to ${options['--record']}`);
    } else {
        const pinned = caseName ? JSON.parse(expected).filter(c => c.name === caseName) : JSON.parse(expected);
        for (let i = 0; i < Math.max(actual.length, pinned.length); i++) {
            const observed = actual[i], baseline = pinned[i];
            if (JSON.stringify(observed) === JSON.stringify(baseline)) continue;
            const name = observed?.name ?? baseline.name;
            const replay = `node tools/verify_visual6502.cjs --suite ${selected.name} --case ${name}`;
            if (!observed || !baseline) throw Error(`${selected.name}: case ${name} missing from ${observed ? 'fixture' : 'driver'}; replay: ${replay}`);
            for (const field of ['name','initial','events']) {
                if (JSON.stringify(observed[field]) !== JSON.stringify(baseline[field])) {
                    throw Error(`${selected.name}: ${name} ${field}: expected ${JSON.stringify(baseline[field])}, actual ${JSON.stringify(observed[field])}; replay: ${replay}`);
                }
            }
            for (let cycle = 0; cycle < Math.max(observed.cycles.length, baseline.cycles.length); cycle++) {
                if (JSON.stringify(observed.cycles[cycle]) !== JSON.stringify(baseline.cycles[cycle])) {
                    throw Error(`${selected.name}: ${name} cycle ${cycle}: expected ${JSON.stringify(baseline.cycles[cycle])}, actual ${JSON.stringify(observed.cycles[cycle])}; replay: ${replay}`);
                }
            }
        }
        if (!caseName && serialized !== expected) throw Error(`${selected.file}: fixture serialization differs despite identical observations`);
        console.log(`Visual6502 revD @ ${manifest.revision}: ${actual.length} ${selected.name} traces reproduced${caseName ? ` (${caseName})` : ''}`);
    }
}
