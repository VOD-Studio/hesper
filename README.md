...
当前 M0 包含 25 个真实实现的 opcode、64 KiB RAM Bus、复位、寄存器快照、指令周期统计、结构化错误和自包含 CLI 演示。具体寻址方式、周期、标志和测试映射见 [opcode 清单](docs/opcodes.md)，其余 opcode 均未实现。
...
尚未实现 ADC/SBC（包括二进制和十进制）、其他官方指令／寻址方式、IRQ/NMI/BRK/RTI、非官方 opcode、机器系统或浏览器前端。BRK `$00` 和其他未支持字节返回 `UnsupportedOpcode { address, opcode }`。
