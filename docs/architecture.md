...
- **PIA 两端口读回路径不同**（`pia.rs`）：Port A 读实际引脚（输出位由 PIA 输出缓冲驱动，引脚电平即 ORA；输入位为外设电平），Port B 输出位读输出锁存、输入位读引脚；PIA 之外对输出线的争用与端口上下拉／浮空不建模。
...
- **不建模**：DRAM 单元电荷保持本身、TTL 传播延迟与单稳态容差、2513 字模与像素／视频合成、滚动引起的帧长变化（D8/D9 的预置位接线已核对，见 H18），以及 PIA 输出线的外部争用与端口上下拉／浮空。逐项边界见 `docs/apple1/hardware-evidence.md`。
