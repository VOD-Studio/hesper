...
- 采用常见后期 NMOS 的正确 ROR 行为，不仿真最早芯片的 ROR 缺陷。BRK/RTI 与周期采样 IRQ/NMI 已实现；RDY/SO 和非官方指令尚未建模。
...
M2.4 当前已用周期采样替代 M1 的指令边界近似。`set_irq_line(true)` / `set_nmi_line(true)` 表示低有效引脚断言；在当前接口中宿主在周期之间改变电平。每周期采样 IRQ 电平及 I，NMI 比较本周期与上次采样电平来锁存断言边沿；完全落在两个采样点之间的短脉冲不被接受。
...
固定 Visual6502 **NMOS revD** 提交 `d8ecc129b34e0eaf320e0400fcf33329475bdb1e` 的 96 个原始程序／引脚事件场景用于交叉核对，共 2304 个总线周期；包含分支、连续改写 I、短脉冲、BRK／IRQ 向量抢占。数据、重现命令及来源许可见 [测试说明](../crates/cpu6502/tests/data/README.md)。不能将这一修订的观察推广到所有 NMOS 芯片。

物理 RESET 引脚的持续断言／释放、RDY、SO 及半周期输入仍待后续 M2.4 完善。`begin_reset` 目前是有明确七周期行为的宿主请求，不冒充完整物理 RESET 引脚模型。寄存器快照不表示包含全部中间状态的存档。
