...
`reset` 采用 NMOS 的七周期入口结果：I 置位，当前 SP 以 8 位回绕减 3，PC 从 `$FFFC` 低字节、`$FFFD` 高字节取得，A/X/Y 和 N/V/D/Z/C 保留，栈不写入。返回 7 周期，不包含保持 RESET 引脚的时间或第一条指令。先对当前 PC 读两次，再读取当前 SP、SP−1、SP−2 对应的栈地址，最后读取向量。`begin_reset()` 启动同一序列，由宿主调用 `cycle` 分次推进；它可放弃未完成指令，但并非物理 RESET 引脚的保持／释放时序。[MOS 手册第 9 章](https://lbaeza.neocities.org/mcs6500/6500_ch09)
...
这次升级实际修改了执行模型。未来完整引脚模型仍需引入采样／同步状态，必要时拆分时钟相位；Bus 外围的宿主负责设备时钟及事件注入。当前逐周期总线通过不等于 IRQ/NMI/RDY/SO 的逐相位兼容认证。
...
`StepKind::Instruction { opcode }` 包含实际取到的 opcode；`Irq` / `Nmi` / `Reset` 是独立入口事件，没有虚构指令字节。IRQ/NMI 保存当前 PC（高字节先压栈）、旧 P（B=0），再置 I，分别读取 `$FFFE/FFFF` 和 `$FFFA/FFFB`。本次 `step` 不执行处理程序首条指令。BRK 则读取填充字节后保存 PC+2、旧 P 且 B=1，不受 I 屏蔽，使用 IRQ 向量。RTI 依次恢复 P、PC 低／高字节，耗用 6 周期。所有栈操作都在 `$0100..01FF` 回绕。

RESET 优先清空已排队 IRQ/NMI，但保留宿主输入电平。因此保持断言的 NMI 不会因 reset 产生新边沿，IRQ 则可在重新清 I 后再次采样。

这是一份明确的**指令边界近似协议**。它不等价于 NMOS 的流水线／引脚采样：没有指令内部边沿、分支末周期延迟、NMI 对 BRK/IRQ 的向量抢占或丢失窗口；也不保证所有连续改写 I 的短指令组合的硬件时序。Visual6502 的研究记录和 Altirra 手册对此有更细的逐相位观察（见 [参考资料](references.md)）。M2.3 已补齐入口的丢弃取指等总线访问；M2.4 仍需要结合执行阶段与宿主时钟重新实现引脚采样时序，不能仅凭本轮中断测试宣称引脚精确。
