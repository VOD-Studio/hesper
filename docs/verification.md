...
第二轮完整试跑 [34949250908](https://github.com/VOD-Studio/hesper/actions/runs/34949250908)
在提交 `1a5fe7d` 上全部通过：六个平台分别完成 CLI 构建、63 项宿主库测试与
实际 demo，常规 debug/release 检查、全量 CPU 对照、Wasm 绑定／浏览器检查
通过。最终汇总得到六个 CLI 压缩包、四个 Wasm 压缩包与 `SHA256SUMS`。
手动 dispatch 的发布步骤按条件跳过；这是构建与打包验证，不是新版本发布，
原 `v0.2.0` tag／Release 保持不变。
