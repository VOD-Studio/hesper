...
Integer BASIC 占据 `$E000–$EFFF` 的 4 KB 区间（需要将第二组内存跳线到 `$E000`，基础 4 KB 配置不能直接运行 BASIC）。BASIC 的 `>` 提示符和经典的 `PRINT` / `GOTO` 命令使 Apple I 从「电子爱好者实验板」变成了可以方便编程使用的个人电脑。Hesper 当前固定安装两组 RAM（`$0000–$0FFF` 与 `$E000–$EFFF`），可用 `--program basic-c.bin --program-address 0xE000` 直接加载镜像，再输入 `E000R` 运行；具体命令见 [`examples.md`](examples.md#加载-integer-basic)。
...
- 8 KiB RAM（`$0000–$0FFF` 与 `$E000–$EFFF` 各 4 KiB）、256 字节 Woz Monitor ROM、开路总线
...
- 可变内存跳线与其他扩展内存映射——当前固定为 `$0000` 和 `$E000` 两组 4 KiB RAM
