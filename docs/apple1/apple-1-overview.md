...
> **地址别名。** 原理图中 A15–A12 与 A4 参与 PIA 片选译码，A3/A5–A11 未参与。因此 `$D014` 与 `$D010` 选中同一端口（Port A），`$D017` 与 `$D013` 选中同一控制寄存器（CRB），依此类推。Woz Monitor 仅使用 `$D010–$D013`，不影响功能验证；Hesper 当前仅映射这四个地址。详见 [`hardware-evidence.md`](hardware-evidence.md)。
