...
典型的输出速率受限于视频移位寄存器的串行移位速率——手册规格表给出 15734 Hz 行频和 60.05 Hz 帧频，但未直接给出每秒最大字符数的单一数值。爱好者社区常推导 "约每秒 60 字符"（与帧频相当）作为保守估计。CPU 在发送下一个字符之前必须轮询显示口的状态标志（`$D012` 的第 7 位），等待移位寄存器准备好接收。

上电时，因为移位寄存器没有硬件清零电路，屏幕会充满不确定图案——通常表现为交替的 `@` 和 `_` 符号，这是移位寄存器 PMOS 动态时钟行为的产物。按下 CLEAR SCREEN 按钮（视频板硬件输入，与 CPU 完全无关）可清屏并将光标移到左上角。

### 键盘与 PIA 握手

Apple I 的键盘与显示通过 **MC6820 Peripheral Interface Adapter（PIA，周边接口适配器）** 连接到 CPU。PIA 是内存映射设备：
...
> **地址别名。** 原理图中 A15–A12 与 A4 参与 PIA 片选译码，A3/A5–A11 未参与。因此 `$D014` 与 `$D010` 选中同一端口（Port A），`$D017` 与 `$D013` 选中同一控制寄存器（CRB），依此类推。Woz Monitor 使用 `$D010–$D013`，这份 BASIC 镜像则通过 `$D0F2` 访问显示器；Hesper 按上述片选条件映射全部别名。详见 [`hardware-evidence.md`](hardware-evidence.md)。
...
Integer BASIC 占据 `$E000–$EFFF` 的 4 KB 区间（需要将第二组内存跳线到 `$E000`，基础 4 KB 配置不能直接运行 BASIC）。BASIC 的 `>` 提示符和经典的 `PRINT` / `GOTO` 命令使 Apple I 从「电子爱好者实验板」变成了可以方便编程使用的个人电脑。Hesper 默认安装两组 RAM（`$0000–$0FFF` 与 `$E000–$EFFF`），可用 `--program basic-c.bin --program-address 0xE000` 直接加载镜像，再输入 `E000R` 运行；具体命令见 [`examples.md`](examples.md#加载-integer-basic)。
...
- 默认 8 KiB RAM（`$0000–$0FFF` 与 `$E000–$EFFF` 各 4 KiB），可选 `$1000–$1FFF` 扩展至 12 KiB、256 字节 Woz Monitor ROM、开路总线
- MC6820/6821 PIA 寄存器模型（DDR、控制寄存器、中断标志、端口数据），并实现 Port B 的 CB2 输出握手：写 ORB 后第一个 E 拉低、CB1 有效沿释放，另含写脉冲（下一个 E 释放）与手动低／高模式
- 40×24 字符屏幕，来自 1024 槽循环存储模型（六片 2504）；物理读头仅随 MEMΦ 前进，屏幕原点由读头与光栅的相对位置推导；CLEAR SCREEN 原子地清空、对齐循环存储并归位光标
- 字符在光标槽被扫到时接受，CR 逐槽清到行尾；D8/D9 按写控制与消隐条件同步加载 `$BF`／`$00`，D15 插入正常帧的六条保持扫描线。正常帧 262 行；隔离的底行 CR／末列折行得到 263 行及额外 40 次存储移位，连续滚动不固定套用该帧长（见硬件依据 H18）
- 键盘 FIFO 输入队列与完整的 CA1 选通脉冲时序
- 逐周期 CPU 推进；主板时钟以 14.31818 MHz 主时钟建模，内存刷新期间抑制 Φ2（CPU 停在 Φ2、PIA 无 E、无总线访问），板时钟、视频与 B3 单稳态继续推进
...
- 可变内存跳线与具体扩展卡电气模型——现支持默认两组 4 KiB RAM 与可选 `$1000–$1FFF` 地址映射
- 视频模拟电路、负载与串扰，以及原板字模完整逐点认证；已实现的 2519 行重放、固定替换字模、D1 像素移位、视频光标与数字复合同步见[视频说明](video.md)
- TTL 器件传播延迟、单稳态容差与真实上电随机态：B3 的 3.5 µs 按标称值量化到整主时钟周期（51 master tick），不是器件认证
- DRAM 单元的电荷保持特性本身（只建模刷新期间的 Φ2 门控）
