...
- 40×24 字符屏幕，来自 1024 槽循环存储模型（六片 2504：960 可见 + 64 消隐）与 40 字符 2519 行缓冲；CLEAR SCREEN 清循环存储并归位光标
- 字符接收按原图时序：DA 在光标槽被扫到时接受，CR 逐槽清到行尾并把光标送到下一行行首，滚动由垂直重载完成
...
- 2513 字符 ROM 字模、D1 像素移位寄存器与视频合成：只投影字符格，不产生像素或复合视频
- TTL 器件传播延迟、单稳态容差与真实上电随机态：B3 的 3.5 µs 按标称值量化到整主时钟周期（51 master tick），不是器件认证
- DRAM 单元的电荷保持特性本身（只建模刷新期间的 Φ2 门控）
- 滚动引起的帧长变化：D8/D9 计数器的预置字位接线已核对（1011 1111 = 191），垂直重载译码与帧长影响仍未建模（见 `docs/apple1/hardware-evidence.md` H18）
