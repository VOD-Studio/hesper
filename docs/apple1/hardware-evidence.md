...
| **实现位置** | `display.rs`：`memory` 保存六条数据轨道（位 0–4 与反相的位 6；位 5 不入库），`host` 另行保存宿主投影字符且不参与控制逻辑。 |
| **来源状态** | **原图推导**：六位存储（六片 2504）；（D6/D7 的若干输入浮空、VINH 两处标法同网）另有原板走线勘误。 |
| **实现关系** | **数字模型**：存储的六位按 2513 的地址接线取「位 0–4 + 反相位 6」；本 crate 不实现 2513 字模与像素输出，宿主文本投影沿用既有大写字母／数字／标点约定并单独保存，以免投影反哺控制逻辑。 |
...
| **实现位置** | `display.rs::Display::accept`：`control = !(ack_n \|\| RD6 \|\| RD7)`。CR（`$0D`，由 RD1/RD3/RD4 为 1、RD2/RD5/RD6/RD7 为 0 解码）确认后逐槽清到行尾并把光标走到下一行；其余控制码完成握手但不写入存储、不移动光标。 |
...
| **实现关系** | **吻合（数字模型）**：接受时刻＝光标槽在该圈扫过的时刻；一圈 1024 槽＝一帧，故同一行内连续写入的两个字符之间通常相隔约一帧（本 crate 的回归测试固定该差值为「一帧 + 一个字符时钟」）；CR 清行期间送入的字符顺延到下一行行首。宿主不得再设置任何「字符延时」参数。 |
...
| **实现位置** | `display.rs`：C7 请求锁存在 DA 的上升沿捕获、被接受时清除；`clear_to_eol` 期间不接受。`pia.rs`：ORB 写只在 CRB 选择输出寄存器时发生；未驱动数据线按 TTL 高解析。RESET 清 ORB/DDRB 并丢弃待发 E 操作。 |
| **来源状态** | **原件未规定**：忙时写入行为、未配置握手下的端口行为、RESET 期间与显示的交叠均未在手册或数据表中规定。 |
| **实现关系** | **模拟约定（可复现）**：一次请求只被接受一次——DA 保持高不会重复写入同一槽；CLEAR SCREEN 与 RESET 都不撤销已发出的请求，但 RESET 清掉 ORB 之后硬件确实不再持有该字符，本模型也不人为保留副本（在途字符随 RESET 丢失，与 PIA 寄存器被清零一致）。 |
...
4. **DRAM 单元衰减**：只建模 Φ2 门控，不建模刷新周期之外的电荷保持特性，也不建模真实上电随机态。

5. **上电屏幕初始状态**：当前为确定性空白（模拟器初始化选择），非移位寄存器随机图案。

6. **开路总线初始值**：`last_read` 初始 0 为模拟约定，真实硬件浮空无此保证。

7. **MC6820 vs MC6821 命名**：历史硬件为 6820；代码以 6821 命名，并以 MC6820 印刷页 47（scan leaf 48）的 Table 5 复核 CB2 数字规则。电气参数与 RESET 细节不能互换。

8. **垂直重载的具体数值**：D8/D9 preset 的十进制值（图上 191）与滚动帧长变化未逐位复现；见 H18。

9. **像素输出与字模**：不实现 2513 字模、D1 像素移位寄存器与视频合成，只投影字符格。
