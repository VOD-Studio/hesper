...
Apple I 主板允许将每个 4 KiB 区域跳线为 RAM、ROM 或 I/O。Hesper 默认两组 RAM 均已安装的配置（`$0000–$0FFF` 与 `$E000–$EFFF` RAM、`$D010–$D013` PIA、`$FF00–$FFFF` ROM），不建模可变跳线。
...
- RAM：默认安装 8 KiB，第一组位于 `$0000–$0FFF`，第二组位于 `$E000–$EFFF`，用于加载 BASIC 等程序；两组互不镜像。

可选 `expansion_ram` 在 `$1000–$1FFF` 增加 4 KiB RAM，总计 12 KiB；它仅提供明确的地址映射，不模拟具体扩展卡的电气、跳线或刷新电路，也不改变现有板级时钟。默认配置和以下主板证据边界保持原义。
